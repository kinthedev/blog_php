    ...
    <div class="flex">
        ...
        <form method="post" class=" w-[35%] h-screen flex flex-col items-center py-25 px-25 space-y-8"
            ...
            <div class="relative w-full">
                ...
                <span class=<?php if (!empty($nameErr)) echo "error" ?>> <?php echo  $nameErr ?></span>
            </div>
            <div class="relative w-full">
                <input class="w-full outline-none  py-1" type="text" name="email" placeholder="Email">
                <span class="w-full absolute left-0 -bottom-1  h-px bg-gray-400"></span>
                <span class=<?php if (!empty($emailErr)) echo "error" ?>><?php echo  $emailErr ?></span>
            </div>
            <div class=" relative w-full">
                <input class="w-full outline-none  py-1" type="password" name="password" placeholder="Password">
                <span class="w-full absolute left-0 -bottom-1  h-px bg-gray-400"></span>
                <span class=<?php if (!empty($passErr)) echo "error" ?>> <?php echo $passErr ?></span>

            </div>
            <div class="relative w-full">
                <input class="w-full outline-none  py-1" type="password" name="confirmPassword"
